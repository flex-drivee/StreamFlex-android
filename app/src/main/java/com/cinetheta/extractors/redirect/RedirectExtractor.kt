package com.cinetheta.extractors.redirect
...
class RedirectExtractor : BaseExtractor() {
    ...
    fun decodeWpRedirect(combined: String): String? {
        return try {
            ...
            val root = com.cinetheta.core.parser.JsonParser.parse(jsonStr)

            // Method 1: "o" parameter directly holds the base64-encoded URL
            val encodedUrl = com.cinetheta.core.parser.JsonParser.string(root, "o")?.trim() ?: ""
            ...
            val data = com.cinetheta.core.parser.JsonParser.string(root, "data")?.trim() ?: ""
            val blogUrl = com.cinetheta.core.parser.JsonParser.string(root, "blog_url")?.trim() ?: ""
            if (data.isNotEmpty() && blogUrl.isNotEmpty()) {
                val decodedData = String(decodeBase64Safe(data), Charsets.UTF_8).trim()
                val req = com.cinetheta.core.network.RequestBuilder()
                    .url("$blogUrl?re=$decodedData")
                    .build()
                return kotlinx.coroutines.runBlocking {
                    when (val resp = com.cinetheta.core.network.HttpClient.execute(req)) {
                        is com.cinetheta.core.network.NetworkResult.Success -> {
