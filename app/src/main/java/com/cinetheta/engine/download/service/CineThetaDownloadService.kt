 ...
 */
class CineThetaDownloadService : Service() {

    companion object {
        const val CHANNEL_ID = "cinetheta_downloads"
        ...
        fun startService(context: Context) {
            val intent = Intent(context, CineThetaDownloadService::class.java).apply {
                action = ACTION_START
            }
    ...
    private fun acquireLocks() {
        try {
            ...
            wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "CineTheta:DownloadWakeLock").apply {
                acquire(12 * 60 * 60 * 1000L) // 12 hours max safety limit
            }

            val wifiManager = applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
            @Suppress("DEPRECATION")
            wifiLock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "CineTheta:DownloadWifiLock").apply {
                acquire()
            }
        } catch (_: Exception) {}
    }
