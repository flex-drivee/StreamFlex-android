...
object AppUpdater {
    ...
    private const val GITHUB_REPO = "StreamFlex"
    ...
    suspend fun checkUpdate(context: Context) = withContext(Dispatchers.IO) {
        try {
            if (GITHUB_OWNER == "YourGithubUsername") {
                com.streamflex.core.utils.StreamLogger.error("AppUpdater", "Please configure GITHUB_OWNER and GITHUB_REPO in AppUpdater.kt")
                return@withContext
            }
            ...
            com.streamflex.core.utils.StreamLogger.error("AppUpdater", "Error checking for updates: ${e.message}")
        }
    }
    ...
    private fun installApk(context: Context, apkFile: File) {
        try {
            ...
            com.streamflex.core.utils.StreamLogger.error("AppUpdater", "Failed to install APK: ${e.message}")
